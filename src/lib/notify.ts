...
export async function notifyPagamentoRecebido(parcelaId: string, valor: number, formaPagamento: string) {
  try {
    const { data: p } = await supabase
      .from("parcelas")
      .select("matricula_id, polo_id, matriculas:matricula_id(aluno_id, alunos:aluno_id(nome, telefone)), polos:polo_id(nome)")
      .eq("id", parcelaId)
      .maybeSingle();
    ...
    try {
      if (alunoTelefone) {
        ...
        await sendConfirmacaoPagamento({ telefone: alunoTelefone, nome: alunoNome, valor });
      }
    } catch (waErr) {

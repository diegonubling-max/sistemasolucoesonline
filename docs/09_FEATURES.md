...
## Módulo: Webinar — simulação completa de "aula ao vivo" em vídeo gravado (26/08-01/09/2026)
...
- ✅ **Histórico completo de entrada/saída** — cada entrada (1ª vez ou reentrada) vira uma sessão nova em `webinar_sessoes`, mesmo reabrindo a mesma aba/navegador (ver BUG-074); admin vê tudo num modal por aluno (botão "Histórico")
- ✅ **Suporte a Panda Video** (30/08/2026) — além do YouTube, aceita link de embed do Panda Video; detecção automática pela URL, toda a engenharia (salto de entrada, depoimentos, contador) funciona igual nos dois provedores. Player sem NENHUM controle nativo visível pro aluno (nem play/pause, nem barra de progresso, nem tempo, nem volume) — só o botão de tela cheia, via parâmetro `controls=fullscreen` no embed (01/09/2026). Botão próprio "Ativar áudio" e o aviso de som só aparecem pro YouTube — o Panda já mostra volume nativo (removido, depois voltou a ficar totalmente sem controles com o `controls=fullscreen`)
- ✅ **Botão de maximizar/tela cheia própria** — resolvidos vários rounds de bugs de posicionamento (ver `14_BUGS_CONHECIDOS.md`, BUG-067) até chegar numa versão que funciona em qualquer tamanho de tela
...
- ✅ **(01/09/2026) Botão "Duplicar" na lista de Webinars** — o Diego mantém um webinar "molde" protegido (**"As 3 formas de concluir os estudos"**, `protegido=true` — ver `03_DATABASE.md`) com o vídeo certo e os depoimentos já importados, nunca clicado em Iniciar. Toda vez que precisa de um webinar novo pra usar/testar, clica em "Duplicar" nele: abre um modal com o nome sugerido "{título} - Cópia" (editável), cria um webinar novo com o mesmo `youtube_url`/`gravado`/`modo_acesso` e copia todos os depoimentos — sem precisar recriar nada nem pedir ajuda
- ⚠️ Vários bugs de sincronização (vídeo travando pausado, depoimentos só aparecendo ao minimizar/reabrir, "gravado" desligado sem querer) foram encontrados e corrigidos ao longo de várias rodadas de teste real do Diego, inclusive com alunos de verdade — ver `14_BUGS_CONHECIDOS.md` (BUG-064 a BUG-076) pro histórico completo de causa raiz de cada um

...
### BUG-083: Agendamentos de prova migrados não apareciam na guia "Aprovados"; tela de Documentação/Certificação totalmente quebrada
- **Como foi descoberto:** Diego pediu pra verificar se os dados de uma migração (alunos com documentação/certificação em lotes 02/03/04-2026, e agendamentos de prova de jan-jun/2026) estavam aparecendo corretamente nas telas "Documentação e Certificação" e "Agendamento de Provas".
- **Parte 1 — Provas Agendadas (✅ corrigido):** as guias da tela (Agendadas/Aprovados/Reprovados/Reagendar) classificam cada linha pelo campo `status`, não pelo `resultado`. Os 100 agendamentos migrados ficaram todos com `status='agendada'` — inclusive os 74 que já tinham `resultado='aprovado'` preenchido. Como a `data_prova` já tinha passado (jan-jun/2026 vs. hoje), a tela jogava os 100 pra guia "Reagendar" em vez de separar 74 em "Aprovados". **Solução:** `UPDATE prova_agendamentos SET status='aprovado' WHERE status='agendada' AND resultado='aprovado'` — 74 linhas corrigidas (23/09/2026). Os outros 26 (sem resultado) permanecem em "Reagendar", que é o correto pra eles. Conferido que esse descompasso status/resultado não existe em nenhum outro registro do sistema fora desses 100.
- **Parte 2 — Documentação e Certificação (⚠️ schema ainda não corrigido — dado migrado foi excluído em vez de corrigido):** essa tela (3 guias: Documentação, Envios para Certificadora, Certificados, em `_admin.setor-provas.tsx`) faz consultas em `documentacao_alunos` usando ~20 nomes de coluna que **não existem** na tabela real do banco — `nome_aluno`, `polo`, `quem_vendeu`, `telefone`, `ctr`, `documentacao_completa`, `declaracao_gerada`, `declaracao_data`, `cert_observacao`, `doc_rg_cpf`, `doc_comprovante_residencia`, `doc_historico_fundamental`, `doc_historico_fundamental_medio`, `doc_outros`, `doc_outros_descricao`, `arquivos_paths`, `cert_digital_enviado`, `cert_fisico_recebido`, `cert_fisico_enviado_aluno`, `cert_fisico_rastreio`, `cert_fisico_data_recebimento`, `updated_at`. A tabela real só tem os nomes antigos (`rg_cpf`, `comp_residencia`, `doc_completa`, `hist_fundamental`, `hist_fund_medio`, `outros`, `rec_firma`, `diario_oficial`, `visto_confere`, `matricula_id`). Não existe nenhuma migração no repositório (`supabase/migrations/`) que cria essas ~20 colunas novas — parece que o código dessa tela foi atualizado/reescrito em algum momento (provavelmente direto no Lovable) mas a migração de banco correspondente nunca chegou a ser criada/aplicada nesse projeto Supabase. **Resultado:** a query falha com erro de Postgres (`42703: column does not exist`) e a tela não carrega absolutamente nada — não é específico dos dados migrados, afeta a aba inteira pra qualquer usuário.
- **Decisão do Diego (24/09/2026):** em vez de corrigir o schema agora, pediu pra excluir toda a migração de teste. Excluídos (SQL direto, transação única, conferido 0 restante depois): os 100 registros de `documentacao_alunos` (lotes 02/03/04-2026), os 100 registros de `alunos` criados só pra essa migração (sem matrícula/pagamento/qualquer outro uso — confirmado antes de excluir) e os 100 registros de `prova_agendamentos` migrados (incluindo os 74 já corrigidos na Parte 1, que deixaram de existir junto com o resto). Ver `11_CHANGELOG.md` (24/09/2026). O bug de schema em si **continua existindo** e vai se repetir se alguém tentar migrar dado novo pra essa tela sem corrigir as colunas antes.
- **Reimportação (24/09/2026):** Diego enviou um CSV novo (563 agendamentos, jan-dez/2026) pra substituir os 100 registros excluídos. Dessa vez o campo `status` foi derivado de `resultado` já na importação (`aprovado`→`aprovado`, `reprovado`→`reprovado`, vazio→`agendada`), evitando repetir o descompasso da Parte 1. Conferido depois: distribuição nas guias bate exatamente com a projeção feita antes de importar (Agendadas=11, Reagendar=72, Aprovados=479, Reprovados=1, total 563). Nenhum CTR do CSV bateu com aluno ativo, então todos entraram com `aluno_id=NULL` (são só histórico de consulta, sem login vinculado). Ver `11_CHANGELOG.md` (24/09/2026).
- **Status:** Parte 1 ✅ Resolvido, dado reimportado corretamente em 24/09/2026 — Parte 2 ⚠️ Schema continua quebrado, sem dado pendente no momento (Documentação e Certificação segue vazia/inutilizável até o schema ser corrigido)

### BUG-084: Provas Agendadas — aluno/telefone/polo/sit. financeira em branco pra agendamentos sem `aluno_id` vinculado
- **Como foi descoberto:** Diego mandou print da guia "Agendadas" mostrando os 563 agendamentos recém-importados com todas as colunas (Aluno, Telefone, CTR, Polo, Sit. Financeira) em "—", mesmo os dados estando certos no banco (`nome_aluno`, `telefone`, `polo`, `situacao_financeira` todos preenchidos pra esses registros)
- **Causa:** em `_admin.provas-agendadas.tsx`, o cálculo de nome/telefone/CTR/polo/sit. financeira só usa os campos próprios do agendamento (`nome_aluno`, `telefone`, `polo`, `ctr`, `situacao_financeira`) quando `is_externo=true`. Pra qualquer registro com `is_externo=false`, o código tenta buscar esses dados só via `aluno_id` → tabela `alunos` — sem nenhum fallback pros campos próprios do agendamento. Os 563 agendamentos migrados são `is_externo=false` (não são "alunos externos" no sentido do sistema — são histórico normal) e têm `aluno_id=NULL` (nenhum CTR do CSV bateu com aluno ativo), então caíam no pior dos dois mundos: nem usavam o `aluno_id` (que não existe) nem os campos próprios (porque só eram lidos quando `is_externo=true`)
- **Corrigido:** nome, CTR, telefone, polo e situação financeira agora fazem fallback pros campos próprios do agendamento (`a.nome_aluno`, `a.ctr`, `a.telefone`, `a.polo`, `a.situacao_financeira`) sempre que não existe `aluno` vinculado (`aluno_id` nulo ou aluno não encontrado) — independente de `is_externo`. Pra agendamentos com `aluno_id` vinculado, o comportamento não muda (continua priorizando o dado ao vivo do cadastro do aluno)
- **Status:** ✅ Resolvido (24/09/2026)

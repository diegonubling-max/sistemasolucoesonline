...
### Fluxo (atualizado 28/07/2026)
1. Aluno preenche dados pessoais (etapa 1)
2. Etapa 2 (única): escolhe forma de pagamento (Boleto/Cartão) e confirma — vai **direto pro pagamento**, sem nenhuma etapa de termo/checkbox no meio (removida a pedido do Diego, era fricção desnecessária no funil)
3. Ao confirmar, gera o registro de aceite (hash SHA-256, código de validação, usando o nome da etapa 1) em segundo plano — sem exigir ação explícita do aluno — e vai direto pra tela de boas-vindas + pagamento (PIX/boleto/cartão via Asaas)
...
- Termo aceito digitalmente em segundo plano — não pedir novamente no admin

...
- **Pendente (depende de outro repositório):** o `/aulao` (que fica em outro site, chat "Site - Github - Vercel") precisa gravar um **cookie compartilhado entre subdomínios** (`solucoes_utm`, `Domain=.supletivosolucoesonline.com.br`) com os UTMs/fbclid capturados na URL — `localStorage` **não funciona** aqui porque `/aulao` (supletivosolucoesonline.com.br) e `/matricula` (sistema.supletivosolucoesonline.com.br) são origens diferentes pro navegador, mesmo sendo subdomínios do mesmo domínio. O lado de leitura já está pronto neste sistema (`lerCookieUtmCompartilhado()` em `matricula.tsx`, 26/07/2026) — só falta o outro repositório gravar o cookie.

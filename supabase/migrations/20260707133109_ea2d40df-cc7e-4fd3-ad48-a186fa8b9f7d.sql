...
-- Add 'parcial' to payment_status enum
ALTER TYPE payment_status ADD VALUE IF NOT EXISTS 'parcial';

-- Grants for parcelas_pagamentos (already exists, ensure access)
GRANT SELECT, INSERT, UPDATE, DELETE ON public.parcelas_pagamentos TO authenticated;
GRANT ALL ON public.parcelas_pagamentos TO service_role;

-- RPC to register a payment (total or partial)
...
GRANT EXECUTE ON FUNCTION public.registrar_pagamento_parcela(uuid, numeric, date, text, integer, numeric, numeric, text) TO authenticated;
